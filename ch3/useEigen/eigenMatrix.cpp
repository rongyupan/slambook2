...
int main(int argc, char **argv) {
  ...
  cout << "sum（所有元素之和）: " << matrix_33.sum() << endl;  // 各元素和
  cout << "trace（迹：主对角线元素之和）: " << matrix_33.trace() << endl;  // 迹
  ...
  // 解方程
  // 我们求解 matrix_NN * x = v_Nd 这个方程，其中 matrix_NN = A*A^T
  ...
  // 这个clock()计时方法比起chrono写起来简单
  cout << "time of normal inverse is "
       << 1000 * (clock() - time_stt) / (double)CLOCKS_PER_SEC << "ms" << endl;
  cout << "x = " << x.transpose() << endl;

  // 矩阵分解的内容得再复习一下数值分析的内容，基本忘光了~
