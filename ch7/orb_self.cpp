...
// 下面会看到其容量为 8，那么总共有 8x32=256 bits
...
int main(int argc, char **argv) {
  ...
  //--第三步：画出匹配结果。plot the matches
  cv::Mat image_show;
  cv::drawMatches(first_image, keypoints1, second_image, keypoints2, matches,
                  image_show);
  cv::imshow("matches", image_show);
  cv::imwrite("matches_self.png", image_show);
  cv::waitKey(0);

  cout << "匹配对比orb_cv少，但是效率更高。\n检测+描述对比：0.004-0.014"
       << endl;
  return 0;
}

// --------------------------------------------------------------------------------------------------
// ORB pattern. 特征点附近取256个点对，作为该点的BRIEF描述子
...
/**
 * 计算描述子：compute descriptor of orb keypoints
 * @param img input image
 * @param keypoints detected fast keypoints
 * @param descriptors descriptors
 *
 * NOTE: if a keypoint goes outside the image boundary (8 pixels), descriptors
 * will not be computed and will be left as empty
 */
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints,
                vector<DescType> &descriptors) {
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2,
             vector<cv::DMatch> &matches) {
  const int d_max = 40;  // 设置阈值

  for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
    if (desc1[i1].empty()) continue;

    // 比较第 i1 和第 0 个描述子，初始化其汉明距离为 256（每个描述子256位）
    ...
    for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
      ...
      for (int k = 0; k < 8; k++) {
        // DescType由8个uint32构成，先取异或，对结果判断位是1的个数；
