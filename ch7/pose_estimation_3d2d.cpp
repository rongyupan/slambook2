...
int main(int argc, char **argv) {
  ...
  // 深度图为16位无符号数，单通道图像；
  Mat d1 = imread(depth_1, CV_LOAD_IMAGE_UNCHANGED);
  Mat K = (Mat_<double>(3, 3) << 520.9, 0, 325.1, 0, 521.0, 249.7, 0, 0, 1);
  vector<Point3f> pts_3d;  // 第一张图的相机坐标系下的三维点
  vector<Point2f> pts_2d;  // 第二张图对应的像素点；
...
void bundleAdjustmentGaussNewton(const VecVector3d &points_3d,
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    for (int i = 0; i < points_3d.size(); i++) {
      ...
      // 下面的导数表达式中用到了 1/z 和 1/z^2，所以先在此定义，方便下面的书写；
    ...
    cout << "iteration: " << iter << ", cost = " << std::setprecision(12)
         << cost << endl;
    if (dx.norm() < 1e-6) {
      // converge
      break;
