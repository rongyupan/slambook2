...
/**
 * 双线性插值。get a gray scale value from reference image (bi-linear
 * interpolated)
 * @param img
 * @param x
 * @param y
...
void OpticalFlowSingleLevel(const Mat &img1, const Mat &img2,
  ...
  OpticalFlowTracker tracker(img1, img2, kp1, kp2, success, inverse,
                             has_initial);
  // calculateOpticalFlow只需要一个参数，用占位符_1传入；
  // ? 和自己认知不一样的是： bind 也传入了 &tracker
    ...
    for (int iter = 0; iter < iterations; iter++) {
      ...
      for (int x = -half_patch_size; x < half_patch_size; x++)
        for (int y = -half_patch_size; y < half_patch_size; y++) {
          ...
          // dx和dy都是浮点数，所以x+dx没法精确定位到某个像素，得用双线性插值；
          double error =
              GetPixelValue(img1, kp.pt.x + x, kp.pt.y + y) -
              GetPixelValue(img2, kp.pt.x + x + dx, kp.pt.y + y + dy);
          ;
          // 求e关于(dx,dy)的偏导。
          // 如果不是反向光流法(inverse=false)，那么只有第二项起作用；
          // 使用求导的定义：令步长为2，就能写出下面的式子；
          if (inverse == false) {
            J = -1.0 *
                Eigen::Vector2d(0.5 * (GetPixelValue(img2, kp.pt.x + dx + x + 1,
                                                     kp.pt.y + dy + y) -
            ...
            // 反向光流法使用 I1(x,y) 的梯度替代
            // in inverse mode, J keeps same for all iterations
            // NOTE this J does not change when dx, dy is updated, so we can
...
void OpticalFlowMultiLevel(const Mat &img1, const Mat &img2,
  ...
  // 从最顶层（第一层，level最大），逐步到最底层（第四层，level最小）
  for (int level = pyramids - 1; level >= 0; level--) {
    ...
    t2 = chrono::steady_clock::now();
    auto time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
    cout << "track pyr " << level << " cost time: " << time_used.count()
         << endl;
