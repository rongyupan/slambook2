...
int main(int argc, char **argv) {
  ...
  int nPoints = 2000;
  int boarder = 20;
  VecVector2d pixels_ref;    // 像素点坐标
  vector<double> depth_ref;  // 像素点对应的深度

  // generate pixels in ref and load depth data
  for (int i = 0; i < nPoints; i++) {
    // 随机选点（避开边界位置）：[boarder, cols-boarder)
...
void JacobianAccumulator::accumulate_jacobian(const cv::Range &range) {
  ...
  for (size_t i = range.start; i < range.end; i++) {
    ...
    double X = point_cur[0], Y = point_cur[1], Z = point_cur[2], Z2 = Z * Z,
    ...
    for (int x = -half_patch_size; x <= half_patch_size; x++)
      for (int y = -half_patch_size; y <= half_patch_size; y++) {
        ...
        J_img_pixel =
            Eigen::Vector2d(0.5 * (GetPixelValue(img2, u + 1 + x, v + y) -
...
void DirectPoseEstimationMultiLayer(const cv::Mat &img1, const cv::Mat &img2,
  ...
  // 金字塔里保存的顺序是：原图 → 1/2 → 1/4 ……
  // 对应于一般的层数叫法是：第4层 → 第3层 → 第2层 ……
  // scales 的顺序也是 4-3-2-1，所以为了保证从粗到细，level是从大到小遍历的；
